...
# ────────────────────────────────────────────────────────
search_cache: Dict[str, tuple] = {} # {query_clean: (data, timestamp)}
        ...
        if USE_SUPABASE and supabase:
            ...
            try:
                ...
                for n in (res_news.data or []):
                    if isinstance(n, dict): # FIXED: Type inference check
                        results.append({
                            "type": "news",
                            "id": n.get("id"),
                            "title": n.get("title"),
            ...
            try:
                ...
                for u in (res_users.data or []):
                    if isinstance(u, dict): # FIXED: Type inference check
                        results.append({
                            "type": "user",
                            "id": u.get("id"),
        ...
        if USE_SUPABASE and supabase:
            ...
            if res.data and len(res.data) > 0:
                ...
                for item in res.data:
                    if isinstance(item, dict): # FIXED: Type inference check
                        crops.append({
                            "id": item.get("id"),
                            "name": item.get("title"),
...
def get_crop_details(crop_name: str):
    ...
    found_key: Optional[str] = None # FIXED: Explicit Optional typing
    ...
    if found_details and found_key is not None: # FIXED: NoneType check for dictionary key assignment
