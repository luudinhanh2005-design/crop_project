...
def get_expert_requests(status: str = Query(None), user_id: str = Query(None), expert_user: dict = Depends(verify_expert_role)):
    ...
    try:
        ...
        # 1. Yêu cầu gửi đích danh cho mình (target_expert_id = expert_id)
        # 2. Yêu cầu chung không chỉ định người nhận (target_expert_id is NULL)
        q = supabase.table("expert_requests").select("*")
        
        if expert_id:
            q = q.or_(f"target_expert_id.eq.{expert_id},target_expert_id.is.null")
...
def get_expert_stats(expert_user: dict = Depends(verify_expert_role)):
    ...
    if not (USE_SUPABASE and supabase):
        return {"success": False}
    try:
        # 1. Đếm số câu trả lời của chuyên gia này
        res_ans = supabase.table("expert_responses").select("id", count=CountMethod.exact).eq("expert_id", expert_user.get("sub")).execute()
        ans_count = res_ans.count or 0
        
        # 2. Tổng lượt quét hệ thống (để chuyên gia biết quy mô)
        res_scans = supabase.table("scan_history").select("id", count=CountMethod.exact).execute()
        scan_count = res_scans.count or 0
