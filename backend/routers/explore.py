...
# Import dữ liệu toàn cục từ config
try:
    from config import CROP_DETAILS, USE_SUPABASE, SUPABASE_URL, SUPABASE_KEY  # type: ignore
    from api import supabase  # type: ignore
except ImportError:
    from backend.config import CROP_DETAILS, USE_SUPABASE, SUPABASE_URL, SUPABASE_KEY
    from backend.api import supabase

# Bộ đếm view in-memory phục vụ cho xu hướng động (Fuzzy popularity score)
in_memory_crop_views = {
    "rice": 1250,
    "corn": 850,
    "wheat": 420,
    "potato": 320,
...
# Dữ liệu thời vụ 12 tháng tại 3 miền Bắc - Trung - Nam (Đưa từ frontend sang backend để động hóa)
SEASONAL_DATA = {
    "Bắc": {
...
# ────────────────────────────────────────────────────────
search_cache = {} # {query_clean: (data, timestamp)}
...
def search_explore(q: str = Query(""), page: int = Query(1), limit: int = Query(10)):
    ...
    try:
        ...
        for crop_id, details in CROP_DETAILS.items():
            vi_name_val = details.get("vi", "")
            vi_name = vi_name_val if isinstance(vi_name_val, str) else ""
            sci_name_val = details.get("ten_khoa_hoc", "")
            sci_name = sci_name_val if isinstance(sci_name_val, str) else ""
            ...
            if best_score > 0.35:
                ...
                img = gallery[0] if isinstance(gallery, list) and gallery else "https://images.unsplash.com/photo-1592841200221-a6898f307baa?auto=format&fit=crop&w=400"
...
def get_trending_crops():
    ...
    for crop_id, views in sorted_views[:5]:
        ...
        vi_name_val = details.get("vi", crop_id.capitalize())
        vi_name = vi_name_val if isinstance(vi_name_val, str) else crop_id.capitalize()
        gallery = details.get("gallery")
        img = gallery[0] if isinstance(gallery, list) and gallery else "https://images.unsplash.com/photo-1530507629858-e4977d30e9e0?auto=format&fit=crop&w=400"
        crops.append({
            "id": crop_id,
            "name": vi_name,
...
def get_crop_details(crop_name: str):
    ...
    for crop_id, details in CROP_DETAILS.items():
        vi_name_val = details.get("vi", "")
        vi_name = vi_name_val if isinstance(vi_name_val, str) else ""
        if crop_id.lower() == crop_name_lower or vi_name.lower() == crop_name_lower:
            found_key = crop_id
            found_details = details.copy()
            break
            
    if found_key is not None and found_details is not None:
